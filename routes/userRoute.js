...
); /* .single('image'): This method specifies that the route expects a single file upload with the field name 'image'. The string 'image' here corresponds to the name attribute of the HTML form input that will be used to upload the file. For example, if you have an HTML form like this:*/
...
user_route.get(
  "/download/:filename",
  auth.isLogin,
  userController.handleDownloadRequest
);

user_route.get(
  "/force-download/:filename",
  auth.isLogin,
  userController.forceDownload
);

// user_route.get("/download/:filename", auth.isLogin, (req, res) => {
