...
const loadHome = async (req, res) => {
  try {
    ...
    res.render("home", { user: userData, files: uploadedFiles });
  } catch (error) {
    console.log(error.message);
  }
};
...
const sentVerificationLink = async (req, res) => {
  try {
    ...
    if (user_Data) {
      ...
      res.render("verification", {
        message: "Reset verification mail sent to your mail id, Please check",
      });
    } else {
...
const handleDownloadRequest = async (req, res) => {
  ...
  try {
    ...
    // Not downloaded before – save and allow download
    await Download.create({ userId, fileId: file._id, filename });
    const filePath = path.join(__dirname, "../public/uploadedFiles/", filename);
...
const forceDownload = async (req, res) => {
  ...
  try {
    ...
    const existing = await Download.findOne({ userId, fileId: file._id });
    if (existing) {
